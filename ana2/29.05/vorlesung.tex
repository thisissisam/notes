...
%
%\begin{proof}
%	Seien $ \varphi : U \to \mathbb{R}^m \text{ und }  \psi : V \to \mathbb{R}^l $ mit 
%	$  $ 
%	$ \lim_{\substack{y \to y_0 \\ y \neq y_0}}\frac{\left| \psi (y) \right| }{\left| y - y_0 \right| } = 0$ 
%	so gewählt, dass gilt  
%		\begin{align}
%			g(x) &= g(x_0) + Dg(x_0)(x-x_0) + \varphi(x) \; \forall x \in U \tag{5}\\
%			f(y) &= f(y_0) + Df(y_0)(y-y_0)+ \psi(y) \; \forall y \in V \tag{6}
%		\end{align}
%		
%		Dann erhält man durch Einsetzen der Gleichung 
%		{\Huge Beweis fehlt im Handy}
%		=========================================
%		Wir betrachten getrennt die beiden $ \tilde{\phi} $ definiertn Summanden : Zunächst ist 
%		$$
%		 \lim_{x \to x_0} \frac{\left| Df(g(x_0)) \phi(x) \right| }{\left| x - x_0 \right| }  = 
%		 \lim_{x \to x_0} \left| Df(g(x_0)) \cdot \left( \frac{\phi(x)}{}  \right)  \right| 
%		$$
%		
%\end{proof}
\smalltitle[]{Korollar}
